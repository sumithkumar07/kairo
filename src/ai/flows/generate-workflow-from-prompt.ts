...
import type { RetryConfig } from '@/types/workflow'; // Import RetryConfig type
...
const WorkflowNodeSchema = z.object({
  ...
  type: z.string().describe('Type of the node (e.g., httpRequest, parseJson, aiTask, sendEmail, databaseQuery, dataTransform, conditionalLogic, executeFlowGroup, forEach, whileLoop). Choose the most appropriate type. Refer to available node types and their specific config requirements.'),
...
const WorkflowConnectionSchema = z.object({
  ...
  sourcePort: z.string().optional().describe("The specific output port/handle on the source node to connect from (e.g., 'response', 'videos', 'result', 'output_data', 'status', 'error_message', 'results', 'iterations_completed'). Refer to the source node type's 'outputHandles' definition. Nodes that can fail (httpRequest, aiTask, databaseQuery, dataTransform, sendEmail, executeFlowGroup, forEach, whileLoop) will output a 'status' (e.g., 'success', 'error') and 'error_message' field if an error occurs."),
...
- Each 'node' represents a distinct step (trigger, action, logic, group, iteration).
- Each 'connection' defines the data flow between nodes.

Node Requirements: For each node, you MUST provide:
- 'id': A unique, snake_case string identifier. Node IDs within an 'executeFlowGroup', 'forEach', or 'whileLoop' node's sub-flow nodes should be unique within that sub-flow.
- 'type': The most appropriate node type. Examples: 'httpRequest', 'parseJson', 'aiTask', 'sendEmail', 'databaseQuery', 'dataTransform', 'conditionalLogic', 'executeFlowGroup', 'forEach', 'whileLoop'. If the user mentions "code" or "script", try to map it to a 'dataTransform' node with a suitable 'transformType' if possible, otherwise use 'aiTask'. For encapsulating a sequence of steps, use 'executeFlowGroup'. For iterating over a list, use 'forEach'. For conditional looping, use 'whileLoop'.
...
5.  Node Type Selection: Be specific. If "script" or "code" is requested, attempt to use a 'dataTransform' type or 'aiTask' if appropriate. For encapsulating steps, use 'executeFlowGroup'. For iterating over lists, use 'forEach'. For conditional looping, use 'whileLoop'.
6.  Configuration: Provide sensible defaults/placeholders. Crucially, use '{{env.VARIABLE_NAME}}' for credentials/sensitive configs and note these in 'aiExplanation'. Ensure configuration for implemented nodes like 'sendEmail', 'databaseQuery', 'dataTransform', 'conditionalLogic', 'executeFlowGroup', 'forEach', 'whileLoop' matches their requirements.
7.  Production Focus: Aim for robust, sensible workflows, including basic error handling considerations and retry mechanisms where appropriate. For 'whileLoop', consider if 'maxIterations' is suitable.
