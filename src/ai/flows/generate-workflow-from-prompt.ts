...
const WorkflowNodeSchema = z.object({
  ...
  type: z.string().describe('Type of the node (e.g., httpRequest, parseJson, aiTask, sendEmail, databaseQuery, dataTransform, conditionalLogic). Choose the most appropriate type. Refer to available node types and their specific config requirements.'),
  ...
  config: z.any().describe("Configuration parameters for the node. This should be a valid JSON object. Populate relevant fields based on the node's 'type' and its corresponding 'configSchema'. Examples:\n- 'httpRequest': include 'url', 'method'. Node output includes 'status_code' and 'response'. On error, includes 'status: \"error\"' and 'error_message'.\n- 'aiTask': include 'prompt', 'model'. Node output includes 'output'. On error, includes 'status: \"error\"' and 'error_message'.\n- 'sendEmail': include 'to', 'subject', 'body'. Expect EMAIL_* env vars for server settings. Node output includes 'messageId'. On error, includes 'status: \"error\"' and 'error_message'.\n- 'databaseQuery': include 'queryText' (SQL with $1, $2 placeholders) and 'queryParams' (JSON array of values). Expect DB_CONNECTION_STRING env var. Node output includes 'results', 'rowCount'. On error, includes 'status: \"error\"' and 'error_message'.\n- 'conditionalLogic': 'condition' string (e.g., '{{data.value}} == \"success\"', '{{data.count}} > 10', '{{prev_node.status}} == \"error\"'). Outputs a boolean 'result'.\n- 'dataTransform': include 'transformType' and specific params. See dataTransform types below. Node output is 'output_data'. On error, includes 'status: \"error\"' and 'error_message'.\n- Use placeholders like '{{previous_node_id.output_handle_name.property}}' to reference outputs. For credentials, PREFER `{{env.YOUR_VARIABLE_NAME}}` and mention this in aiExplanation. For conditional execution, include a '_flow_run_condition' field with a placeholder pointing to a boolean output, e.g., '{{conditional_node.result}}' (which MUST be a boolean)."),
  aiExplanation: z.string().optional().describe('AI-generated explanation for this node: why it was chosen, its configuration (especially data flow placeholders, credential placeholders like `{{env.YOUR_VARIABLE_NAME}}` to be set by user, conditional execution fields using `_flow_run_condition: "{{id_of_conditional_node.result}}"` which expects a boolean, and specific config for sendEmail/databaseQuery/dataTransform like parameters for $1,$2 or transformType), assumptions made, and its role. Explain if error handling is considered (e.g., checking `{{prev_node.status}} == "error"`).'),
});
...
const WorkflowConnectionSchema = z.object({
  ...
  sourcePort: z.string().optional().describe("The specific output port/handle on the source node to connect from (e.g., 'response', 'videos', 'result', 'output_data', 'status', 'error_message'). Refer to the source node type's 'outputHandles' definition. Nodes that can fail (httpRequest, aiTask, databaseQuery, dataTransform) will output a 'status' (e.g., 'success', 'error') and 'error_message' field if an error occurs."),
...
- 'type': The most appropriate node type. Examples: 'httpRequest', 'parseJson', 'aiTask', 'sendEmail', 'databaseQuery', 'dataTransform', 'conditionalLogic'. If the user mentions "code" or "script", try to map it to a 'dataTransform' node with a suitable 'transformType' if possible, otherwise use 'aiTask'.
...
- 'aiExplanation': Your detailed explanation for this node: why chosen, config details (data flow, env vars, conditions, specific params for complex nodes like 'sendEmail', 'databaseQuery', 'dataTransform'), assumptions, role. Crucially, if this node's execution depends on the success/failure of a previous node, or if its own failure might need to be handled, explain how the workflow addresses this using conditional logic and checks on 'status' or 'error_message' outputs.

Connection Requirements: For each connection:
- 'sourceNodeId', 'targetNodeId'.
- 'sourcePort', 'targetPort': (Recommended) Specify output/input handles (e.g., 'response', 'output_data', 'result' for conditionalLogic, 'status', 'error_message' for error checking).
...
6.  Configuration: Provide sensible defaults/placeholders. Crucially, use \`{{env.VARIABLE_NAME}}\` for credentials/sensitive configs and note these in \`aiExplanation\`. Ensure configuration for implemented nodes like 'sendEmail', 'databaseQuery', 'dataTransform', 'conditionalLogic' matches their requirements.
...
const generateWorkflowFromPromptFlow = ai.defineFlow(
  ...
  async input => {
    ...
    if (output.nodes.length === 0 && input.prompt.trim() !== "") {
        ...
        // Potentially return a minimal error node or ask for clarification
    }
    return output;
  }
);
