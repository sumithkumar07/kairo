...
const WorkflowNodeSchema = z.object({
  ...
  type: z.string().describe('Type of the node (e.g., httpRequest, parseJson, aiTask, sendEmail, databaseQuery, dataTransform, conditionalLogic, executeFlowGroup, forEach, whileLoop, parallel, manualInput). Choose the most appropriate type. Refer to available node types and their specific config requirements.'),
...
const WorkflowConnectionSchema = z.object({
  ...
  sourcePort: z.string().optional().describe("The specific output port/handle on the source node to connect from (e.g., 'response', 'videos', 'result', 'output_data', 'status', 'error_message', 'results', 'iterations_completed'). Refer to the source node type's 'outputHandles' definition. Nodes that can fail (httpRequest, aiTask, databaseQuery, dataTransform, sendEmail, executeFlowGroup, forEach, whileLoop, parallel, manualInput) will output a 'status' (e.g., 'success', 'error', 'partial_success') and 'error_message' field if an error occurs."),
...
- 'type': The most appropriate node type. Examples: 'httpRequest', 'parseJson', 'aiTask', 'sendEmail', 'databaseQuery', 'dataTransform', 'conditionalLogic', 'executeFlowGroup', 'forEach', 'whileLoop', 'parallel', 'manualInput'. If the user mentions 'code' or 'script', try to map it to a 'dataTransform' node with a suitable 'transformType' if possible, otherwise use 'aiTask'. For encapsulating a sequence of steps, use 'executeFlowGroup'. For iterating over a list, use 'forEach'. For conditional looping, use 'whileLoop'. For concurrent execution of multiple independent task sequences, use 'parallel'. For steps requiring human intervention or data entry, use 'manualInput'.
...
- 'config': A JSON object for node-specific parameters.
    ...
    - Simulation Configuration (Optional): For nodes with external side-effects, you can provide simulation data. This allows testing workflow logic without actual external calls when the workflow is run in 'simulation mode'.
        - 'httpRequest': Add 'simulatedResponse' (JSON object, can include 'status_code') and 'simulatedStatusCode' (number) to 'config'.
    ...
    - Specific Node Config Examples:
        ...
        - 'manualInput': For representing human interaction. Config must include:
            ...
            - 'simulatedResponse': A JSON string representing the data this node will output during simulation (e.g., '{"field_name": "simulated value", "another_field": true}'). This should match the structure defined by 'inputFieldsSchema'.
            The 'manualInput' node outputs an object: '{ "output": <parsedSimulatedResponse>, "status": "success/error", "error_message": "..." }'.
- 'aiExplanation': Your detailed explanation for this node: why chosen, config details (data flow, env vars using '{{env.VAR}}' or conceptual vault secrets '{{secret.VAR}}', conditions, specific params for complex nodes like 'sendEmail', 'databaseQuery', 'dataTransform' (including its subtypes like 'reduceArray' and its 'reducerFunction'), 'conditionalLogic', 'executeFlowGroup', 'forEach' (including '{{item.property}}' usage and 'continueOnError' behavior), 'whileLoop' (including 'condition'/'maxIterations' and how loop state is managed), 'parallel' (including 'branch' structure and 'concurrencyLimit'), 'manualInput' (including 'instructions', 'inputFieldsSchema' for form generation, and 'simulatedResponse' for testing), any 'retry' config, any 'onErrorWebhook' config including its potential use for integrating with a Dead-Letter Queue (DLQ) or triggering an error-handling workflow using the 'workflow_data_snapshot_json', and any simulation-related configs like 'simulatedResponse' for 'httpRequest', 'simulatedMessageId' for 'sendEmail', 'simulatedResults'/'simulatedRowCount' for 'databaseQuery', or 'simulatedOutput' for 'aiTask'), assumptions, role. Crucially, if this node's execution depends on the success/failure of a previous node, or if its own failure might need to be handled, explain how the workflow addresses this using conditional logic and checks on 'status' or 'error_message' outputs, or via the 'onErrorWebhook' for DLQ patterns. For 'executeFlowGroup', 'forEach', 'whileLoop', or 'parallel' branches, explain the sub-flow definition, mappings, and purpose of the group/loop/branch. For 'manualInput', explain that 'simulatedResponse' allows the workflow to be tested, and a real system would pause for user interaction. For nodes with simulation configs, explain their purpose for testing.
...
Analyze the user's request carefully and construct the complete workflow. Be meticulous about node types, configurations (with data flow placeholders, credential placeholders, conditional execution fields, retry configurations, 'onErrorWebhook' configurations including their potential use for DLQ/error workflow integration, simulation-related config fields like 'simulatedResponse', 'simulatedOutput', and specific configs for 'executeFlowGroup', 'forEach' (including '{{item.property}}' usage and 'continueOnError'), 'whileLoop' (including 'condition'/'maxIterations' and how loop state is managed), 'parallel' (including 'branch' structure and optional 'concurrencyLimit'), 'manualInput' (including 'instructions', 'inputFieldsSchema', and 'simulatedResponse'), 'dataTransform' (including 'reduceArray' type and its config like 'reducerFunction' and 'initialValue')), and connections (with port/handle names where appropriate). Explicitly design error paths using 'conditionalLogic' to check for 'status: "error"' from preceding nodes, or use 'onErrorWebhook' for direct external error notification and DLQ integration. For 'manualInput' steps, explain that the 'simulatedResponse' allows the workflow to run end-to-end in the current system, but in a production scenario with pause/resume, this would be a point of actual human interaction.
`,
});
