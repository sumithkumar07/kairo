...
function initGroqClient(): Groq {
  if (groqClient) return groqClient;
  
  const apiKey = process.env.GROQ_API_KEY || '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';
...
export async function chatWithGroq(
  ...
  try {
    console.log('[GROQ] Making chat request with', messages.length, 'messages');
    ...
    console.log('[GROQ] Received response successfully');
    ...
    console.error('[GROQ] Error in chatWithGroq:', error);
    throw error;
  }
}
...
The output MUST be a single, valid JSON object that represents the workflow with 'name', 'description', 'nodes', and 'connections' fields.`;
  ...
  try {
    console.log('[GROQ] Raw response content:', response.content);
    ...
    console.log('[GROQ] Cleaned content for parsing:', cleanContent.substring(0, 200) + '...');
    
    return JSON.parse(cleanContent);
  } catch (error) {
    console.error('[GROQ] Error parsing workflow JSON:', error);
    console.error('[GROQ] Raw response that failed to parse:', response.content);
    throw new Error('Failed to parse workflow JSON from GROQ response');
  }
}
